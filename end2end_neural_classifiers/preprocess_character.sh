#!/bin/bash
#python --version
#python -c 'import torch;print("Pytorch version is "+torch.__version__)'

# You should change the input_dir here:
input_dir="cross_validation_2class_balanced"
#input_dir="cross_validation_5class"

# to be separated with word-level embedding
output_dir="2class_balanced_ptfiles_char"
#output_dir="5class_ptfiles_char"

OUTDIR="dataset/k-folds-normalized/${output_dir}"

if [ ! -d "${OUTDIR}" ]; then
    mkdir ${OUTDIR}
fi

for i in 1 2 3 4 5
do
    python -u preprocess_character.py \
        -train_src dataset/k-folds-normalized/${input_dir}/source_train_${i}.txt \
        -train_tgt dataset/k-folds-normalized/${input_dir}/target_train_${i}.txt \
        -train_labels dataset/k-folds-normalized/${input_dir}/label_train_${i}.txt \
        -valid_src dataset/k-folds-normalized/${input_dir}/source_test_${i}.txt \
        -valid_tgt dataset/k-folds-normalized/${input_dir}/target_test_${i}.txt \
        -valid_labels dataset/k-folds-normalized/${input_dir}/label_test_${i}.txt \
        -vocab dataset/polyglot_embeddings/predefined_dicts.char.fr-en.pt \
        -min_word_count 0 \
        -save_data dataset/k-folds-normalized/${output_dir}/fold${i}.multidata.char.fr-en.pt
done


