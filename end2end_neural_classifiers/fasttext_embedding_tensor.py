...
src_emb_tensor_path = 'dataset/en-fr-corpus/fasttext-normalized/embedding_en.pt'
tgt_emb_tensor_path = 'dataset/en-fr-corpus/fasttext-normalized/embedding_fr.pt'
...
with open(src_emb_txt, 'r') as src_txt, open(tgt_emb_txt, 'r') as tgt_txt:
    ...
    tgt_word2emb = {}
    for line in src_txt:
        if line.split()[0] == '</s>':  # the symbol for end of sentence
...
for w in src_word2emb:
    src_embedding_tensor[src_word2idx[w]] = src_word2emb[w]     # each word index corresponds to its embedding 
for w in tgt_word2emb:
...
torch.save(src_embedding_tensor,src_emb_tensor_path)
torch.save(tgt_embedding_tensor,tgt_emb_tensor_path)
