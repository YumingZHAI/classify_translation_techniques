...
class DataLoader(object):
    ...
    def __init__(
            ...
            src_insts=None, tgt_insts=None,labels=None,
    ...
    def shuffle(self):
        ...
        if self._labels:
            paired_insts = list(zip(self._src_insts, self._tgt_insts,self._labels))
            random.shuffle(paired_insts)
            self._src_insts, self._tgt_insts, self._labels = zip(*paired_insts)
        else:
    ...
    def next(self):
        ...
        def pad_to_longest(insts):
            ...
            # inst_data: e.g. [   2   87  563    3    0    0    0    0    0    0    0]
            # 2: <s> BOS, 3: </s> EOS
            inst_data = np.array([
                inst + [Constants.PAD] * (max_len - len(inst))
                for inst in insts])

            # inst_position: e.g. [1 2 3 4 5 6 7 8 0]
        ...
        if self._iter_count < self._n_batch:
            ...
            if not self._labels: 
                ...
                inst_data = np.array([int(inst[0]) for inst in labels])
                labels_data_tensor = Variable(torch.LongTensor(inst_data))
                if self.cuda:
                    labels_data_tensor = labels_data_tensor.cuda()
                return (src_data, src_pos), (tgt_data, tgt_pos),(labels_data_tensor)
        else:
            if self._need_shuffle:
                self.shuffle()
