...
emb_tensor_src_path = 'dataset/polyglot_embeddings/embedding_src.pt'
emb_tensor_tgt_path = 'dataset/polyglot_embeddings/embedding_tgt.pt'
...
for word in words_src[4:]:
    ...
    for char in word.lower():
        if char in vectors_src:
            # do a sum on the existing word embedding vector, keep the dimension 64
            # increase the count
            vectors_src[char] = (vectors_src[char][0] + vec, vectors_src[char][1] + 1)
        else:
            vectors_src[char] = (vec, 1)    # the word embedding where the character appears 
...
# TODO: take embedding values for EOS, BOS and PAD from embeddings_src and embeddings_tgt
src_embedding_tensor = torch.FloatTensor(src_embedding_tensor)
tgt_embedding_tensor = torch.FloatTensor(tgt_embedding_tensor)
torch.save(src_embedding_tensor,emb_tensor_src_path)
torch.save(tgt_embedding_tensor,emb_tensor_tgt_path)
